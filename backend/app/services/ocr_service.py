...
class ReceiptOCRService:
    ...
    def parse_items(self):
        lines = [line.strip() for line in self.raw_text.strip().splitlines() if line.strip()]
        i = 0
        while i < len(lines):
            line = lines[i].lower()
            ...
            if ':' in line:
                ...
                quantity_match = re.match(r"^(\d+)\s*(x\s*)?(.*)", lines[i], re.IGNORECASE)
                if quantity_match:
...
def table_recognizer(image_path: str):
    ...
    public_url = upload_excel_to_supabase_storage(
        file_path,
        filename
    )
    if public_url:
