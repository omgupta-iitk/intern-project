...
from ..models.message import User, Message
import os
...
class Query:
    @strawberry.field
    async def messages(self, info: Info, receiverId: Optional[int] = None) -> List[MessageType]:
        ...
        if receiverId:
            ...
            query = supabase.table("messages")\
                ...
                .or_(f"and(sender_id.eq.{current_user.id},receiver_id.eq.{receiverId}),and(sender_id.eq.{receiverId},receiver_id.eq.{current_user.id})")\
...
class Mutation:
    @strawberry.mutation
    async def sendMessage(self, info: Info, receiverId: int, content: str) -> MessageType:
        current_user = await get_user_from_token(info)
        if not current_user:
            raise Exception("Not authenticated")
        ...
        receiver_data = supabase.table("users").select("id").eq("id", receiverId).execute()
        ...
        message_data = supabase.table("messages")\
            .select("id, content, sender_id, receiver_id, created_at, is_read")\
            .eq("id", message_id)\
            .single()\
            .execute()
        ...
        sender_data = supabase.table("users").select("id, fullName").eq("id", current_user.id).single().execute()
        receiver_data = supabase.table("users").select("id, fullName").eq("id", receiverId).single().execute()
