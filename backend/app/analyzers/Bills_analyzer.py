...
class BillsAnalysis:
    def __init__(self, bill_list):
        ...
        self.df["Total"] = self.df["Total"].str.replace(r"[^\d.]", "", regex=True).astype(float)
        self.df["Guests"] = self.df["Guests"].str.replace(r"[^\d.]", "", regex=True).astype(int)
    ...
    def analyze(self):
        ...
        return {
            "bill_metrics": {
                ...
                "total_revenue": float(self.total_revenue()),  # Convert numpy.float64 to native float
